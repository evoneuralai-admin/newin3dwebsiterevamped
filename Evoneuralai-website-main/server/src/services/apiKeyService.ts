...
export async function validateApiKey(rawKey: string): Promise<ValidatedApiKeyUser | null> {
  if (!isFirebaseInitialized() || !db) {
    console.error('Database not available for API key validation');
    return null;
  }
  ...
  for (const doc of snapshot.docs) {
    ...
    if (isValid) {
      ...
      // Get subscription info - check multiple possible fields
      ...
      if (subscriptionDoc.exists) {
        ...
        // Get credits from subscription usage or user document
